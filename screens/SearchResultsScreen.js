...
const SearchResultsScreen = ({ route, navigation }) => {
  ...
  const handleSendRequest = async () => {
    ...
    try {
      const res = await axios.post("/bookings", {
        ...
        payment_mode: packageDetails.paymentMethod, // ✅ Correct (Laravel expects `payment_mode`)
      })
  ...
  const renderTripCard = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <View style={styles.vehicleRow}>
          <View style={styles.vehicleIconContainer}>
            <MaterialCommunityIcons name="truck-outline" size={20} color="#fff" />
          </View>
          <View>
      ...
      <TouchableOpacity style={styles.bookButton} onPress={() => openConfirmationModal(item)} activeOpacity={0.8}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {trips.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Image source={require("../assets/icon.png")} style={styles.emptyImage} resizeMode="contain" />
          <Text style={styles.emptyTitle}>No Trips Found</Text>
          <Text style={styles.emptyText}>
            We couldn't find any trips matching your search criteria. Please try different locations or dates.
          </Text>
          <TouchableOpacity style={styles.emptyButton} onPress={() => navigation.goBack()} activeOpacity={0.8}>
          ...
          <Animated.View style={[styles.modalBox, { opacity: fadeAnim }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalScrollContent}>
              ...
              <View style={styles.sectionCard}>
                ...
                {renderRow("Price", `Rs ${packageDetails.amount}`)}
                {renderRow("Payment Method", packageDetails.paymentMethod === "khalti" ? "Khalti" : "Cash")}
              </View>
            </ScrollView>
          ...
          <View style={styles.modalBox}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.trackingContainer}>
                ...
                >
                  <Text style={styles.trackingNumber}>{bookingInfo?.tracking_no || "Unavailable"}</Text>
